...
public class MirrorControlService implements MirrorControl {
	...
	public void setSittingHeight(double height) {
		m_torso = height * 10;
		s_logger.info("SET[torso=" + m_torso + "]");		
	}
...
System.out.println("---------------------- start -----------------------------------");		
		if ( m_torso < 0 ) {
			System.out.println("------------------------- end -----------------------------------" + m_torso);					
			return;
		}
		...
		m_msm.Set_MSM_PT_Correction(0, 0, m_panOffset, m_tiltOffset);		
		MSM_PT_Property adjust = m_msm.Get_MSM_PT(head);
		s_logger.info("ADJUST(pan=" + adjust.RM_Panning_P + ", tilt=" + adjust.RM_Tilting_P + ")");
		
		m_service.setPan(adjust.RM_Panning_P);
		m_service.setTilt(adjust.RM_Tilting_P);
		System.out.println("---------------------- end -----------------------------------" + m_torso);				
	}
